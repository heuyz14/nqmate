...
Dashboard shows the next important event; released events show actual, consensus, previous, surprise, and point-in-time availability; vintage-aware history is used where available. The current provider layer includes BLS series, the official BLS release calendar, FRED/ALFRED, and BEA normalized observations. Explicit release linking, raw surprise persistence, and event-specific interpretation are covered by service tests.
