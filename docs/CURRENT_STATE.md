...
- The first immutable research manifest is versioned at `docs/data/research-manifest-through-2026-09-08.json`. It records the certification hash and Git revision and correctly remains `certified: false` while 107 quarantines remain.
- Snapshot phase started: `nqmate_api.market.snapshots` defines the six V2 ET snapshot times and fail-closed availability/closed-minute filtering. Persistence and feature-family wiring remain the next slice; quarantined sessions stay excluded.
- Snapshot persistence slice added: migration `023_session_feature_snapshots.sql` is applied, with immutable `SessionFeatureSnapshot` metadata and server-only insert repository support. Feature-family wiring remains separate because retrieved historical availability must not be presented as live point-in-time knowledge.
- Snapshot feature wiring added: `market_feature_function` applies the same cutoff independently to NQ and ES, derives deterministic technical features, and preserves nullable warm-up values plus 5-minute relative strength.
- Snapshot availability policy added: migration `024_snapshot_availability_policy.sql` and policy-aware builders support strict or explicitly labeled event-time reconstruction without rewriting raw bar metadata. Migration 024 must be applied before persisting reconstructed snapshots.
- Snapshot population job added at `jobs/populate_feature_snapshots.py`. The bounded reconstructed run verified 6 snapshots on 2025-01-02 and the resumable range reached 2025-06-20 (588 `market-features-v2.1` snapshots) before the runner window ended; rerun in date chunks to continue through 2026-09-08. Existing snapshots are immutable and safely reused.
