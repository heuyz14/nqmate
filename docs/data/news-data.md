...
For scheduled releases, retain actual, forecast, and previous; calculate raw surprise as `actual - forecast` and preserve its event-specific interpretation. Official BLS/Fed/BEA values override aggregator facts. Deduplicate equivalent reports across sources using semantic similarity, event type, entities, publication-time proximity, numeric values, and source; prefer official sources for canonical facts. The current deterministic clustering baseline stores a stable logical-event key using event type, entities, topics, and a 30-minute publication bucket; provider wording is not treated as identity.
...
The current optional `GeminiNewsExtractor` returns structured event fields and is wrapped by an in-process provider/ID cache during one ingestion run. Enable it only with `NEWS_NLP_ENABLED=true` and a server-side `GEMINI_API_KEY`; deterministic rules remain the default. The default model is configurable through `GEMINI_MODEL` and currently uses `gemini-3.6-flash`. Model output is validated before it can replace normalized fields.
