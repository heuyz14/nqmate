...
async def run(start: date, end: date, train_sizes: tuple[int, ...], test_size: int) -> dict[int, dict[str, dict[str, float | None]]]:
    ...
    rows = rows_from_sessions(sessions, "return_60m")
...
def main() -> None:
    ...
    print(json.dumps(asyncio.run(run(args.start, args.end, train_sizes, args.test_size)), sort_keys=True), flush=True)
