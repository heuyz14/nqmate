...
from nqmate_api.ml.boosted import LightGBMClassifier, SklearnGradientBoostingClassifier, XGBoostClassifier
...
def evaluate_walk_forward(rows: Sequence[LabeledRow], min_train_size: int, test_size: int = 1, include_xgboost: bool = False, include_all_boosting: bool = False) -> dict[str, dict[str, float | None]]:
    include_boosting = include_xgboost or include_all_boosting
    boosting_names = ("xgboost", "sklearn_gradient_boosting", "lightgbm") if include_all_boosting else ("xgboost",)
    names = ("majority", "always_long", "overnight_direction", "logistic", *boosting_names) if include_boosting else ("majority", "always_long", "overnight_direction", "logistic")
    ...
    for train, test in walk_forward_splits(rows, min_train_size, test_size):
        ...
        if include_boosting:
            if len(set(train_labels)) < 2:
                boosted_predictions = {name: [majority_probability(train_labels)] * len(test) for name in boosting_names}
            else:
                models = {"xgboost": XGBoostClassifier}
                if include_all_boosting:
                    models.update({"sklearn_gradient_boosting": SklearnGradientBoostingClassifier, "lightgbm": LightGBMClassifier})
                boosted_predictions = {
                    name: models[name]().fit(tuple(row.features for row in train), train_labels).predict_probability(tuple(row.features for row in test))
                    for name in boosting_names
                }
            for name in boosting_names:
                predictions[name].append((test_labels, boosted_predictions[name]))
